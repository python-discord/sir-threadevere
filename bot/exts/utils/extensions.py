...
from bot import exts, logger
from bot.bot import ThreadBot
from bot.constants import Colours, Emojis, MODERATION_ROLES, Roles, URLs
...
class Action(Enum):
    ...
    LOAD = functools.partial(ThreadBot.load_extension)
    UNLOAD = functools.partial(ThreadBot.unload_extension)
    RELOAD = functools.partial(ThreadBot.reload_extension)
...
class Extensions(commands.Cog):
    ...
    async def list_command(self, ctx: Context) -> None:
        ...
        for category, extensions in sorted(categories.items()):
            ...
            embed.add_field(name=category, value=extensions)

        logger.debug(f"{ctx.author} requested a list of all cogs. Returning an embeded list.")
